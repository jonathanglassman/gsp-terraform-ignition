...
module "k8s-ci-node-ignition" {
  source                = "../k8s-node-ignition"
  dns_service_ip        = "${cidrhost(var.service_cidr, 10)}"
  node_labels           = "node-role.kubernetes.io/ci"
  node_taints           = "node-role.kubernetes.io/ci=:NoSchedule"
  cluster_domain_suffix = "${var.cluster_domain_suffix}"
  k8s_tag               = "${var.k8s_tag}"
  kubelet_kubeconfig    = "${var.kubelet_kubeconfig}"
  kube_ca_crt           = "${var.kube_ca_crt}"
...
data "template_file" "ci-user-data-object-key" {
  template = "/user_data/$${cluster_name}-ci.userdata"
...
data "ignition_config" "ci" {
  files = ["${module.k8s-ci-node-ignition.ignition_file_ids}"]

  systemd = ["${concat(
        module.k8s-ci-node-ignition.ignition_systemd_unit_ids,
        module.common.ignition-systemd-unit-ids,
    )}"]
}
...
resource "aws_s3_bucket_object" "ci-user-data" {
  bucket  = "${var.user_data_bucket_name}"
  key     = "${data.template_file.ci-user-data-object-key.rendered}"
  content = "${data.ignition_config.ci.rendered}"
}

data "ignition_config" "ci-actual" {
  replace = {
    source       = "s3://${var.user_data_bucket_name}${aws_s3_bucket_object.ci-user-data.id}"
    verification = "sha512-${sha512(data.ignition_config.ci.rendered)}"
  }
}
